...
Below is `cleanup_expired_sessions` from `tests/v175-c2/fixtures/dc-job.arcana` — a verified multi-effect declaration. A `dc-job` wraps an effect-annotated function as a scheduled job; this one declares both `Database` and `Cron` effects in its row. The compiler enforces that the body performs no effects beyond those two.

<Tabs syncKey="arcana-code-view">
  <TabItem label="Canonical (S-expression)">
```scheme
;; v1.7.5 C2 fixture: dc-job wrapping a Cron effect fn.
(dc-job
  :decl (dc-fn
...
If the flip above sounds appealing, the next question is whether *your* project is the kind Arcana is built for. [When to use Arcana](/when-to-use-arcana/) is the honest fit guide — what it's strong at, and what it deliberately isn't. A [cookbook](/cookbook/) of worked examples is on the way, gated on the tooling that lets every recipe carry verified canonical rather than hand-guessed forms.
