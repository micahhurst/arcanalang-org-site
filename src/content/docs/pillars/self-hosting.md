...
- **The WASM-GC codegen migration to Arcana is complete.** Across the v1.6.x sprint cycle the WASM-GC code-emission path was rewritten from a Rust shim (`gc_codegen.rs` ~1.2k LOC + `postprocessor.rs` ~6.5k LOC of pure WAT-text manipulation) into a pure-Arcana implementation in `src/wasm.arcana` (~7.6k LOC). At **v1.7.0** the Arcana path became the default and authoritative compile path (D373 Phase 3, WP-43 phase3-routing-flip). The Rust shim is `#[deprecated]` and gated behind `--emit-shim`, an emergency-rollback flag that is itself now sunset (a no-op alias that emits a deprecation banner when invoked); shim deletion was scheduled v1.7.1, slipped to a later sub-version, and is disclosed here rather than removed silently.
