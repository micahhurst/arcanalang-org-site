...
:::tip[Where to start]
This is the first technical pillar to read. No prerequisites — start here, then proceed to [Effect Contracts & Capability Discipline](/pillars/effect-contracts/) → [Compile-Time Safety](/pillars/compile-time-safety/) for the deeper PL theory. Unfamiliar terms? See the [Glossary](/glossary/).
:::
...
// :mode "server" → server-mapped (the schema's Database parameterization
// is the same one declared in function effect rows as Database(server)).
```
  </TabItem>
</Tabs>
